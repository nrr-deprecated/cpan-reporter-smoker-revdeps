...
sub run {
    ...
    my @results =
      map { have_tested( grade => $_, %args ) } qw( FAIL NA UNKNOWN );

    say "### loading CPANDB";
    load_cpandb(1);

    say
"### loading dist objects by name, discarding those without CPANDB entries";
...
sub new_prog_bar {
    ...
    my $params =
      { name => $name, ETA => 'linear', max_update_rate => '0.00001' };
    $params->{count} = $count;
...
sub get_dist {
    ...
    my $dist = eval { CPANDB->distribution($name) };
